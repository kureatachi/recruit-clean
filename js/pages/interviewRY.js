...
// Get all title elements - including specific section IDs
const allTitles = document.querySelectorAll('.title-border .title, h2.title, .title-border h2.title, #section-2, #section-4, #section-5, #interview-section-4 .title-border .title, #interview-section-4 .title-border h2.title, #interview-section-4 h2.title, #interview-section-5 .title-border .title, #interview-section-5 .title-border h2.title, #interview-section-5 h2.title, #interview-section-2 .title-border .title, #interview-section-2 .title-border h2.title, #interview-section-2 h2.title, .white-text-card h2, .leader-overlay h2, #interview-section .white-text-card h2, #interview-section-3 .leader-overlay h2');

// Only apply below 768px - set to 16px
if (width <= 768) {
  // Apply to specific section IDs first (highest priority)
  const section2 = document.querySelector('#section-2');
  if (section2) {
    section2.style.setProperty('font-size', '16px', 'important');
    section2.style.setProperty('text-align', 'center', 'important');
  }
  
  const section4 = document.querySelector('#section-4');
  if (section4) {
    section4.style.setProperty('font-size', '16px', 'important');
    section4.style.setProperty('text-align', 'center', 'important');
  }
  
  const section5 = document.querySelector('#section-5');
  if (section5) {
    section5.style.setProperty('font-size', '16px', 'important');
    section5.style.setProperty('text-align', 'center', 'important');
  }
  
  // Fix white-space for section-3
  const section3 = document.querySelector('#section-3');
  if (section3) {
    section3.style.setProperty('white-space', 'normal', 'important');
  }
...
if (window.innerWidth <= 750) {
  ...
  if (skillCard) {
    ...
    // Override font-size for paragraphs and strong tags in skill card
    const skillParagraphs = skillCard.querySelectorAll('p');
    skillParagraphs.forEach(function(p) {
      p.style.setProperty('font-size', '15px', 'important');
      const strongTags = p.querySelectorAll('strong');
      strongTags.forEach(function(strong) {
        strong.style.setProperty('font-size', '15px', 'important');
      });
    });
    
    // Override font-size for h3 title in skill card
    const skillTitle = skillCard.querySelector('h3');
    if (skillTitle) {
      skillTitle.style.setProperty('font-size', '18px', 'important');
    }
  }
}
}
...
if (window.innerWidth <= 750) {
  ...
  // Desktop: restore to original 22px
  if (changeTitle) {
    changeTitle.style.setProperty('font-size', '22px', 'important');
  }
  
  if (messageTitle) {
    messageTitle.style.setProperty('font-size', '22px', 'important');
  }
  
  if (interviewSection2Title) {
    interviewSection2Title.style.setProperty('font-size', '22px', 'important');
  }
}
}
